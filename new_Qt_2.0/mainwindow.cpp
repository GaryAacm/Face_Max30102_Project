...
void MainWindow::setupPlot()
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        // 设置线条的颜色
        line->setPen(plot->graph(i)->pen());
        line->start->setParentAnchor(plot->axisRect()->topRight());
        line->start->setCoords(-20, -i * 20 - 10);
        line->end->setParentAnchor(plot->axisRect()->topRight());
        line->end->setCoords(-60, -i * 20 - 10);

        textLabel->setPositionAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        textLabel->position->setType(QCPItemPosition::ptAxisRectRatio);
        textLabel->position->setParentAnchor(line->start);
        textLabel->position->setCoords(-5, 0);
        textLabel->setText(plot->graph(i)->name());
