...
func main() {
	...
	// 接收扫码后的 SampleID 和 UserUUID 并暂存
	r.POST("/collect/register", func(c *gin.Context) {
		var req struct {
		...
		// 缓存 SampleID 和 UserUUID
		inMemoryCache.Store(req.CollectID, req.UserUUID)

		c.JSON(http.StatusOK, gin.H{"message": "Collect ID registered", "timeout": 10})
	})

	// 设备端发送 SampleID 和 DeviceID，验证并创建样本
	r.GET("/collect/start", func(c *gin.Context) {
		collectID := c.Query("collect_id")
		deviceID := c.Query("device_id")

		// 从缓存中获取前端传来的 SampleID 和 UserUUID
		userUUID, exists := inMemoryCache.Load(collectID)
		if !exists {
			c.JSON(http.StatusNotFound, gin.H{"error": "Collect ID not found"})
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{"message": "Sample started"})
	})

	// 终止采集请求
	r.GET("/collect/stop", func(c *gin.Context) {
		collectID := c.Query("collect_id")
		var sample Sample

		// 从数据库中查找采样记录并更新
		if err := DB.Where("sample_id = ?", collectID).First(&sample).Error; err != nil {
			c.JSON(http.StatusNotFound, gin.H{"error": "Sample not found"})
			return
		}

		// 更新采集结束时间和持续时间（假设持续时间计算方式）
		sample.Duration = int(time.Since(sample.Timestamp).Seconds())
		DB.Save(&sample)

		c.JSON(http.StatusOK, gin.H{"message": "Sample stopped", "duration": sample.Duration})
	})
